...
if(live) {
  Sys.sleep(.1)
  d <- read.serialConnection(ser_conn) # exclude the header of the data -- difficult to plot text!
  Sys.sleep(.1)
...
ui <- shinyUI(fluidPage(
  theme = shinytheme("readable"), # theme is entirely optional
  ...
  sidebarLayout(
    sidebarPanel(
      checkboxGroupInput("grps", "Channels to use:",
                         names(dat)[-1], selected = names(dat[-1])), # all columns except first
      sliderInput("nsamples", "Samples into the past:",
...
server <- shinyServer(function(input, output){
  ...
  fetchData <- reactive({
    ...
    if(live){ # getting data from the serial port
      dat <<- rbind(dat, 
  ...
  fetchData %>% 
    ...
    layer_lines() %>% 
    scale_numeric("y", domain = reactive(input$yaxis), nice = TRUE, clamp = TRUE) %>%  
